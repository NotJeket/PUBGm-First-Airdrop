...
def update_team_info(n):
    ...
    # Find the player who got the first airdrop
    first_airdrop_player = None
    for player_info in total_player_list:
        if player_info.get("gotAirDropNum") == 1:
            first_airdrop_player = player_info
            break

    if first_airdrop_player is not None and not kill_screen_displayed:
        player_name = first_airdrop_player.get("playerName", "")
        team_name = first_airdrop_player.get("teamName", "")
